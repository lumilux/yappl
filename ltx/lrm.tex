...
There are two built-in functions in YAPPL: \texttt{rand} and \texttt{print}. These are both reserved keywords. 
\subsubsection{rand}
\texttt{rand} takes no arguments and returns a sufficiently random number. We implement the function using [some implementation to be decided.]

\subsubsection{print}
Since YAPPL does not use the \texttt{string} type nor include string literals, printing must be achieved through other means. \texttt{print} takes a single argument that is a list of integers. For each number encountered in the list, \texttt{print} interprets this as the decimal representation of an ASCII character and prints to standard out the corresponding letter character in the ASCII table. Since \texttt{print} has no return value, it is called by itself without the \texttt{\midtilde} function evaluator. \\
\begin{verbatim}
# prints "Hello World" to stdout
print [72, 101, 108, 108, 111, 32, 87, 111, 114, 100]
\end{verbatim}
