...
A pattern consisting of a constant matches the values equal to that constant. Valid pattern constants are: literals of type \term{int}, \term{bool}, or \term{float}, and the empty list (\term{[]}).
...
Since YAPPL does not currently support the \term{string} type or string literals, or allow for side-effects (excepting \term{rand} and \term{seed}), printing must be achieved explicitly within the language. The \term{print} function takes a single expression of one of the three basic types (\term{int}, \term{bool}, and \term{float}) or a list of one of those three types as an argument and prints a string representation of that argument to standard output. \term{print\_line} functions like \term{print} but appends a newline.
