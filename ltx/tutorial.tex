...
This creates a function \texttt{add} that returns an integer and takes in two integers as arguments. The body of the function is after the \texttt{=} sign, and it is allowed to be called after \texttt{in}. The last line of a function is its return value. Recursion is permitted: a function can call itself within its own body. There is no need to explicitly define a function as recursive. 

Functions are called using the function evaluation operator, tilde (\midtilde). 

\subsubsection{Built-in functions}

The above code also demonstrates a built-in function of YAPPL, \texttt{print\_line}, which takes in a single argument to print. \texttt{rand} and \texttt{seed} are other built-in functions that we will use later in this tutorial.

\subsection{Expressions}

Function and variable declarations and definitions are expressions, so are function evaluations. Sequences of expressions are separated by semicolons, and the \texttt{if} statement will be useful in this tutorial:
\begin{floatbox}
\begin{alltt}
if x < 10 then
...
\$ ocamlc -c geom.ml
\$ ocamlc -o geom unix.cma builtin.cmo geom.cmo
\$ ./geom
\$ 8
\end{alltt}
